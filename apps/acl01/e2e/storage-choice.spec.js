...
test.describe('Where your todos are stored', () => {
	...
	// The shared component has no catalogue of its own: it asks the chapter's
	// translator through the registry and falls back to the English written at
	// its call site. Asserting the German is what tells the two apart — a failed
	// registration would render a perfectly plausible English sentence.
	test.describe('in German', () => {
		test.use({ locale: 'de-DE' });

		test('the legend comes from this chapter, not from the fallback', async ({ page }) => {
			test.setTimeout(timeout * 2);
			await page.goto('/');
			await waitForConsent(page);
			await expect(page.getByText('Wo Ihre Aufgaben liegen', { exact: true })).toBeVisible({
				timeout
			});
		});
	});
