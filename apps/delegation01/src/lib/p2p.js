...
async function createOrbitDBInstance(heliaNode) {
	...
	if (!activePasskeyCredential) {
		ownDidStore.set(null);
		// Without identities of our own, `createOrbitDB` builds a keystore under
		// `./orbitdb/keystore`, which browser-level writes to IndexedDB -- the
		// signing key, on a device that was promised nothing would be kept.
	...
	// `useKeystoreDID`, so it encrypted a key pair that signed nothing.
